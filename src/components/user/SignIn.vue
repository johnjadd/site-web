...
<template>
  <div class="login" id="login">
    ...
    <div class="log-bg">
      ...
      <div class="log-logo">欢迎来到云监控</div>
      <div class="log-text">@John Jadd</div>
    </div>
    <div class="log-email">
      <input type="text" placeholder="用户名" :class="'log-input' + (username==''?' log-input-empty':'')"
             v-model="username"><input type="password" placeholder="登录密码"
  ...
  export default {
    ...
    methods: {
      ...
      toLogin() {
        ...
        //请求后端,比如:
        setTimeout(() => {
          this.signInFromApi(loginParam, this.$store).then(ret => {
            if (ret === "failed") {
              alert("Sign in site from server failed!");
              this.isLoging = false;
            } else {
              console.log("Sign in site FromServer done!");
              let {message, token} = ret;
              if (message === "ok") {
                this.$store.commit("setToken", token);
                window.localStorage.setItem('token', token);
                //this.$store.commit('setUser', loginParam.username);

                this.$router.push({name: 'dev', params: {devId: -1} });
              }
              this.isLoging = false;
            }
          });
